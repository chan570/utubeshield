...
import { Search, Trash2, EyeOff, CheckCircle, AlertCircle, Sparkles, MessageSquareOff } from 'lucide-react';
import ReplyModal from './ReplyModal';

export default function CommentTable({ comments = [], videoTitle, commentsDisabled = false }) {
  ...
  return (
    <div className="glass-panel rounded-2xl p-6 border-2 border-peach-200 bg-white shadow-md">
      ...
      {/* Disabled Comments Warning Banner */}
      {commentsDisabled ? (
        <div className="p-8 text-center bg-rose-50 border-2 border-rose-300 rounded-2xl shadow-sm my-4">
          <div className="w-14 h-14 rounded-full bg-rose-100 border-2 border-rose-400 flex items-center justify-center mx-auto mb-3 text-rose-700">
            <MessageSquareOff className="w-7 h-7 text-rose-700" />
          </div>
          <h4 className="text-xl font-black text-rose-950 mb-1.5">Comments are Turned Off for this YouTube Video</h4>
          <p className="text-xs text-rose-900 font-bold max-w-md mx-auto leading-relaxed">
            The creator has turned off public comments for this video on YouTube. Public comment analysis and AI response suggestions are unavailable for videos with disabled comment sections.
          </p>
        </div>
      ) : (
        /* Table */
